...
//Below are edited by myself
//Notice that all comparisons are case INSENSITIVE
...
* of edits required to transform one string to another. An "edit" can
* be an insertion of a single character, a deletion of a single
* character, and a substitution of a single character.
*
* This notion of an "edit" distance can be used to compare the similarity
* of two strings. If two strings have a small Levenshtein distance, then
...
Datum levenshtein_distance(PG_FUNCTION_ARGS)
{
	...
	char* s = TextDatumGetCString(str_01);
	char* t = TextDatumGetCString(txt_02);
	...
	for (j = 0; j <= n; ++j)		//init the first line
		d[j] = j;

	for (i = 1; i <= m; ++i)
	{
		int leftUpper = d[0];		//d[i-1][0]
		d[0] = i;					//now d[0] is d[i][0]
		for (j = 1; j <= n; ++j)
		{
			int left = d[j - 1];	//d[i][j-1]
			int upper = d[j];		//d[i-1][j]
			if (tolower(s[i - 1]) == tolower(t[j - 1]))
				d[j] = leftUpper;
			else
				d[j] = min3(leftUpper, left, upper) + 1;

			leftUpper = upper;		//update to d[i-1][j](next loop d[i-1][j-1])
		}
	}
...
* Jaccard Index
*
* The Jaccard index is another way to represent the similarity of
* two strings. Specifically, the Jaccard index is computed over the
* two sets of bigrams of the two strings.
*
* Given two sets A and B, the Jaccard index is the size of the set
* intersection of A and B, divided by the size of the set union of A
* and B. Remember, a set does not have duplicates.
*
*/
int hashfunc(char x, char y)
{
	return (x - ' ') * 96 + (y - ' ');
}

static int h[9216];
static int clean[210];
...
Datum jaccard_index(PG_FUNCTION_ARGS)
{
	...
	int i, j, pos = 0;
	int interSize = 0, unionSize = 2;

	h[hashfunc('$', tolower(s[0]))] = 1;			//beginning and ending
	h[hashfunc(tolower(s[m - 1]), '$')] = 1;
	clean[pos++] = hashfunc('$', tolower(s[0]));
	clean[pos++] = hashfunc(tolower(s[m - 1]), '$');

	for (i = 1; i < m; ++i)
	{
		if (h[hashfunc(tolower(s[i - 1]), tolower(s[i]))] == 0)
			++unionSize;
		h[hashfunc(tolower(s[i - 1]), tolower(s[i]))] = 1;		//avoid duplicate
		clean[pos++] = hashfunc(tolower(s[i - 1]), tolower(s[i]));
	}


	if (h[hashfunc('$', tolower(t[0]))] == 1)			//beginning
		++interSize;
	else
		++unionSize;
	if (h[hashfunc(tolower(t[n - 1]), '$')] == 1)		//ending
		++interSize;
	else
		++unionSize;
	...
	for (j = 1; j < n; ++j)
	{
		if (h[hashfunc(tolower(t[j - 1]), tolower(t[j]))] == 1)
		{
			++interSize;
			h[hashfunc(tolower(t[j - 1]), tolower(t[j]))] = 2;		//should only intersect once
		}
		else if (h[hashfunc(tolower(t[j - 1]), tolower(t[j]))] == 0)
		{
			++unionSize;
			h[hashfunc(tolower(t[j - 1]), tolower(t[j]))] = 3;		//should only union once
			clean[pos++] = hashfunc(tolower(t[j - 1]), tolower(t[j]));		//notice that these should be cleaned as well
		}
	}


	for (i = 0; i < pos; ++i)		//restore
		h[clean[i]] = 0;

	float4 result = (float)interSize / (float)unionSize;
